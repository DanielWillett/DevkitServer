...
/* Objects are not part of the Level Hierarchy
[EarlyTypeInit]
public sealed class ObjectItemTypeIdentifier : IHierarchyItemTypeIdentifier
{
    private static readonly Type LevelObjectType = typeof(LevelObject);
    public Type Type => LevelObjectType;
    public AssetReference<ObjectAsset> Object { get; private set; }
    public AssetReference<ItemAsset> Buildable { get; private set; }
    public byte TypeIndex => 3;

    internal ObjectItemTypeIdentifier() { }
    public ObjectItemTypeIdentifier(ObjectAsset obj)
    {
        Object = obj == null ? default : obj.getReferenceTo<ObjectAsset>();
    }
    public ObjectItemTypeIdentifier(ItemAsset buildable)
    {
        Buildable = buildable == null ? default : buildable.getReferenceTo<ItemAsset>();
    }
    public ObjectItemTypeIdentifier(AssetReference<ObjectAsset> obj)
    {
        Object = obj;
    }
    public ObjectItemTypeIdentifier(AssetReference<ItemAsset> buildable)
    {
        Buildable = buildable;
    }
    public ObjectItemTypeIdentifier(AssetReference<ObjectAsset> obj, AssetReference<ItemAsset> buildable)
    {
        Object = obj;
        Buildable = buildable;
    }
    public ObjectItemTypeIdentifier(ObjectAsset? obj, ItemAsset? buildable)
    {
        Object = obj == null ? default : obj.getReferenceTo<ObjectAsset>();
        Buildable = buildable == null ? default : buildable.getReferenceTo<ItemAsset>();
    }
    ...
    public void Instantiate(Vector3 position, Quaternion rotation, Vector3 scale)
    {
        ...
        if (!Object.isValid && !Buildable.isValid)
            return;
        ObjectAsset? @object = Object.Find();
        ItemAsset? buildable = Buildable.Find();
        if (@object == null && buildable == null)
            return;
        LevelObjects.registerAddObject(position, rotation, scale, Object.Find(), Buildable.Find());
    }
    public void Write(ByteWriter writer)
    {
        bool objectIsValid = Object.isValid, buildableIsValid = Buildable.isValid;
        writer.Write(objectIsValid);
        writer.Write(buildableIsValid);

        if (objectIsValid)
            writer.Write(Object.GUID);
        if (buildableIsValid)
            writer.Write(Buildable.GUID);
    }
    public void Read(ByteReader reader, ushort version)
    {
        bool hasObjAsset = reader.ReadBool();
        bool hasBuildableAsset = reader.ReadBool();

        Object = hasObjAsset ? new AssetReference<ObjectAsset>(reader.ReadGuid()) : default;
        Buildable = hasBuildableAsset ? new AssetReference<ItemAsset>(reader.ReadGuid()) : default;
    }
    public override string ToString() => "[HIID] Object: todo";
} */
